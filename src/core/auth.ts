...
function expiryDate(value: unknown, index: number, path: string): Date | "Infinity" | null {
  if (typeof value === "number") {
    ...
    if (!Number.isFinite(date.getTime())) {
      fail(index, "expiry is outside the supported date range", path);
    }
    return date;
  }
...
export class Auth {
  ...
  private async readCookieFile(): Promise<unknown[]> {
    ...
    try {
      ...
      throw new AuthError(`failed to read cookie file: ${this.cookiesPath}`, {
    ...
    try {
      ...
      throw new AuthError(`cookie file is not valid JSON: ${this.cookiesPath}`, {
        path: this.cookiesPath,
      });
    }
    ...
    throw new AuthError(
      `cookie file must contain a JSON array or {cookies: []}: ${this.cookiesPath}`,
      { path: this.cookiesPath },
    );
  }
  ...
  private async importCookies(entries: unknown[]): Promise<void> {
    for (let index = 0; index < entries.length; index++) {
      ...
      try {
        ...
        throw new AuthError(`invalid cookie at index ${index} (${this.cookiesPath})`, {
          path: this.cookiesPath,
        });
      }
    }
  ...
  private toJarCookie(entry: unknown, index: number): JarCookie | null {
    ...
    if (typeof raw.value !== "string" || !COOKIE_VALUE.test(raw.value)) {
      return fail(index, "value contains invalid cookie characters", this.cookiesPath);
    }

    for (const key of ["secure", "httpOnly", "hostOnly", "session"]) {
      if (hasOwn(raw, key) && typeof raw[key] !== "boolean") {
        return fail(index, `${key} must be boolean`, this.cookiesPath);
      }
    }
    ...
    if (hasDomain && typeof raw.domain !== "string") {
      return fail(index, "domain must be a string", this.cookiesPath);
    }
    const rawDomain = typeof raw.domain === "string" ? raw.domain.trim().toLowerCase() : undefined;
    if (rawDomain === "") return fail(index, "domain must not be empty", this.cookiesPath);
    let domain: string;
    try {
    ...
    const defaultHostOnly = !rawDomain?.startsWith(".");
    const hostOnly =
      !hasDomain ||
      (raw.hostOnly === undefined ? defaultHostOnly : raw.hostOnly === true);
    if (!hasDomain && raw.hostOnly === false) {
      return fail(index, "domain-less cookies must be host-only", this.cookiesPath);
    }

    if (hasOwn(raw, "path") && typeof raw.path !== "string") {
      return fail(index, "path must be a string", this.cookiesPath);
    }
    const cookiePath = typeof raw.path === "string" && raw.path !== "" ? raw.path : "/";
    if (!cookiePath.startsWith("/") || INVALID_PATH_CHARS.test(cookiePath)) {
      return fail(index, "path is invalid", this.cookiesPath);
    }
